...
def transpose_chord(chord, semitones):
    ...
    root = get_root_note(chord)
    suffix = chord[len(root):]

    if root not in NOTES_SHARP:
        return chord

    old_index = NOTES_SHARP.index(root)
    new_index = (old_index + semitones) % 12

    return NOTES_SHARP[new_index] + suffix


def transpose_chord_sheet(chord_sheet, semitones):
    return [
        [transpose_chord(chord, semitones) for chord in bar]
        for bar in chord_sheet
    ]
...
def calculate_transpose_steps(original_key, target_key):
    original_root = get_key_root(original_key)

    if original_root not in NOTES_SHARP or target_key not in NOTES_SHARP:
        return 0

    return NOTES_SHARP.index(target_key) - NOTES_SHARP.index(original_root)

def save_analysis(filename, audio_info):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()

    cursor.execute("""
        INSERT INTO analysis_history 
        (filename, bpm, song_key, time_signature, mood, chords)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (
        filename,
        audio_info["tempo"],
...
def index():
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            save_analysis(filename, audio_info)
...
def chord_sheet_page():
    transpose_steps = int(request.args.get("transpose", 0))
    target_key = request.args.get("key", None)
    ...
    if target_key:
        transpose_steps = calculate_transpose_steps(
            latest_audio_info["key"],
            target_key
        )
    ...
    transposed_sheet = transpose_chord_sheet(chord_sheet, transpose_steps)

    original_key = latest_audio_info["key"]
    original_mode = original_key.split()[1] if len(original_key.split()) > 1 else ""
    current_key_root = NOTES_SHARP[
        (NOTES_SHARP.index(get_key_root(original_key)) + transpose_steps) % 12
    ]
    current_key = current_key_root + " " + original_mode

    return render_template(
        "chord_sheet.html",
        chord_sheet=transposed_sheet,
        filename=latest_filename,
        time_signature=latest_audio_info["time_signature"],
        key=current_key,
        notes=NOTES_SHARP,
        transpose_steps=transpose_steps,
        selected_key=current_key_root
    )
...
def chord_detail_page(chord_name):
    return render_template("chord_detail.html", chord_name=chord_name)
...
def download_chord_sheet():
    ...
    # Apply transpose to chord sheet
    chord_sheet = transpose_chord_sheet(chord_sheet, transpose_steps)
    ...
    if original_root in NOTES_SHARP:
        current_key_root = NOTES_SHARP[
            (NOTES_SHARP.index(original_root) + transpose_steps) % 12
        ]
        current_key = current_key_root + " " + original_mode
    ...
    details = (
        f"Key: {current_key}    |    "
        f"Time Signature: {latest_audio_info['time_signature']}    |    "
        f"BPM: {latest_audio_info['tempo']}    |    "
        f"Transpose: {transpose_steps:+d}"
    )
    ...
    for i in range(0, len(chord_sheet), bars_per_line):
        ...
        for bar in row:
            ...
            line_parts.append(formatted_bar)
